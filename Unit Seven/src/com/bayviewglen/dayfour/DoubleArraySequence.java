...
public class DoubleArraySequence implements Cloneable {
	...
	public void advance() {
		...
		if (currentIndex == manyItems) {
			throw new IllegalStateException("No current element to advance from");
		}
		currentIndex++;
	}
	...
	public static DoubleArraySequence catenation(DoubleArraySequence s1, DoubleArraySequence s2) {
		...
		DoubleArraySequence newSequence = new DoubleArraySequence(s1.getCapacity() + s2.getCapacity());
	 ...
	 **/
	public boolean isCurrent() { // See if the sequence has a specified current
									// element
